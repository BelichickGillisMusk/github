...
export const xaiRouter: Router = Router();

xaiRouter.post("/chat/completions", async (req: AuthedRequest, res) => {
  const apiKey = process.env.XAI_API_KEY;
  if (!apiKey) {
    res.status(503).json({ error: "xai_key_not_configured" });
    return;
  }
  ...
  try {
    ...
    logger.error({ err: (err as Error).message }, "xai_upstream_failed");
    res.status(502).json({ error: "xai_upstream_failed" });
  }
});
