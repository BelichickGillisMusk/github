...
function ContractorsPage({ onBack }: { onBack: () => void }) {
  const [filter, setFilter] = useState<'all' | ContractorStatus | 'fleet'>('all');
  const [search, setSearch] = useState('');

  const filtered = CONTRACTOR_DATA.filter((c: Contractor) => {
    if (search) {
      const q = search.toLowerCase();
      if (!c.company.toLowerCase().includes(q) && !c.city.toLowerCase().includes(q)) return false;
    }
    if (filter === 'all') return true;
    if (filter === 'fleet') return c.vehicles >= 5;
    return c.status === filter;
  });

  const totalRevenue = CONTRACTOR_DATA.reduce((s, c) => s + c.revenue, 0);
  const totalTests = CONTRACTOR_DATA.reduce((s, c) => s + c.ytd, 0);
  const totalVehicles = CONTRACTOR_DATA.reduce((s, c) => s + c.vehicles, 0);
  const activeCount = CONTRACTOR_DATA.filter(c => c.status === 'active').length;

  const statusColors: Record<ContractorStatus, string> = {
    active: 'text-emerald-400 bg-emerald-400/10 border-emerald-400/30',
    pending: 'text-amber-400 bg-amber-400/10 border-amber-400/30',
    inactive: 'text-zinc-500 bg-zinc-500/10 border-zinc-500/30',
  };

  const exportCSV = () => {
    const header = 'ID,Company,City,Service Type,Vehicles,Last Test,Revenue,YTD Tests,Status\n';
    const rows = CONTRACTOR_DATA.map(c =>
      `"${c.id}","${c.company}","${c.city}","${c.type}",${c.vehicles},"${c.lastTest}",${c.revenue},${c.ytd},"${c.status}"`
    ).join('\n');
    const blob = new Blob([header + rows], { type: 'text/csv' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'NorCalCARB_Contractors_YTD.csv';
    a.click();
  };

  return (
    <div className="min-h-screen bg-brand-black text-zinc-100 font-sans">
      {/* Top bar */}
      <div className="border-b border-white/5 bg-brand-black/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <span className="font-bold text-white tracking-tight">Contractor Accounts</span>
              <span className="ml-2 text-xs text-brand-orange font-mono uppercase tracking-widest opacity-80">Revenue Tracker</span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="hidden md:block text-xs font-mono text-zinc-500 uppercase tracking-widest border border-white/10 px-3 py-1.5 rounded-lg">
              TESTER ID: IF530523
            </span>
            <button
              onClick={exportCSV}
              className="flex items-center gap-2 bg-brand-orange/90 hover:bg-brand-orange text-white px-4 py-2 rounded-xl text-sm font-semibold transition-all shadow-[0_0_15px_rgba(249,115,22,0.25)]"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
          {[
            { label: 'Total Revenue YTD', value: '$' + totalRevenue.toLocaleString(), sub: `${CONTRACTOR_DATA.length} accounts`, accent: 'text-emerald-400' },
            { label: 'Tests Completed', value: totalTests.toString(), sub: 'YTD 2025–2026', accent: 'text-brand-orange' },
            { label: 'Active Accounts', value: activeCount.toString(), sub: `of ${CONTRACTOR_DATA.length} total`, accent: 'text-sky-400' },
            { label: 'Avg Rev / Account', value: '$' + Math.round(totalRevenue / CONTRACTOR_DATA.length).toLocaleString(), sub: 'all accounts', accent: 'text-violet-400' },
            { label: 'Fleet Vehicles', value: totalVehicles.toString(), sub: 'units served', accent: 'text-zinc-300' },
          ].map(k => (
            <div key={k.label} className="glass-card p-5 relative overflow-hidden">
              <div className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mb-2">{k.label}</div>
              <div className={`text-2xl font-bold font-mono ${k.accent}`}>{k.value}</div>
              <div className="text-[11px] text-zinc-600 mt-1 font-mono">{k.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
          <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 mr-1">Filter:</span>
          {(['all', 'active', 'pending', 'inactive', 'fleet'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider border transition-all ${
                filter === f
                  ? 'bg-brand-orange border-brand-orange text-white shadow-[0_0_12px_rgba(249,115,22,0.35)]'
                  : 'border-white/10 text-zinc-400 hover:border-brand-orange/50 hover:text-brand-orange'
              }`}
            >
              {f === 'fleet' ? 'Fleet 5+' : f}
            </button>
          ...
          <input
            ...
            className="ml-auto bg-white/5 border border-white/10 text-zinc-200 placeholder-zinc-600 text-sm px-4 py-1.5 rounded-xl outline-none focus:border-brand-orange/50 font-mono w-52"
          />
        </div>
        ...
        <div className="glass-card overflow-hidden">
            ...
            <table className="w-full min-w-[900px]">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.02]">
                  {['#', 'Company', 'Service Type', 'Vehicles', 'Last Test', 'Revenue', 'YTD Tests', 'Status'].map(h => (
                    <th key={h} className="px-4 py-3 text-left text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-500 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((c, i) => (
                  <tr key={c.id} className={`border-b border-white/[0.04] transition-colors hover:bg-white/[0.03] ${i % 2 === 1 ? 'bg-white/[0.01]' : ''}`}>
                    <td className="px-4 py-3 text-[11px] font-mono text-zinc-600">{c.id}</td>
                    <td className="px-4 py-3">
                      <div className="font-semibold text-white text-sm">{c.company}</div>
                      <div className="text-[11px] font-mono text-zinc-500 mt-0.5">{c.city}, CA</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-[11px] font-mono font-semibold text-zinc-400 bg-white/5 border border-white/10 px-2.5 py-1 rounded-md whitespace-nowrap">{c.type}</span>
                    </td>
                    <td className="px-4 py-3 font-mono text-sm font-bold text-zinc-200">{c.vehicles}</td>
                    <td className="px-4 py-3 font-mono text-xs text-zinc-400">{c.lastTest.replace(/-/g, '/')}</td>
                    <td className="px-4 py-3 font-mono text-sm font-bold text-emerald-400">${c.revenue.toLocaleString()}</td>
                    <td className="px-4 py-3 font-mono text-sm font-bold text-zinc-200">{c.ytd}</td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${statusColors[c.status]}`}>
                        {c.status}
                      </span>
                    </td>
                  </tr>
          ...
          {/* Totals row */}
          <div className="flex justify-end gap-px border-t border-white/10 bg-white/[0.02]">
            {[
              { label: 'Showing', value: `${filtered.length} accounts` },
              { label: 'Vehicles', value: filtered.reduce((s, c) => s + c.vehicles, 0).toString() },
              { label: 'Revenue', value: '$' + filtered.reduce((s, c) => s + c.revenue, 0).toLocaleString(), green: true },
              { label: 'Tests', value: filtered.reduce((s, c) => s + c.ytd, 0).toString() },
            ].map(t => (
              <div key={t.label} className="px-6 py-4 text-right border-l border-white/5">
                <div className="text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-600 mb-1">{t.label}</div>
                <div className={`font-mono text-lg font-bold ${t.green ? 'text-emerald-400' : 'text-white'}`}>{t.value}</div>
              </div>
            ))}
          </div>
        </div>
...
export default function App() {
  ...
  const [page, setPage] = useState<'main' | 'contractors'>(() =>
